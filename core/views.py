...
def repartidor_pedidos_view(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # REPARTIDOR SOLO PUEDE: listo -> en_camino -> entregado
            estados_permitidos = ['en_camino', 'entregado']
            if nuevo_estado not in estados_permitidos:
                messages.error(request, 'Estado no permitido para repartidor.')
                return redirect('repartidor_pedidos')

            # Validar flujo: solo puede pasar de listo->en_camino o en_camino->entregado
            if nuevo_estado == 'en_camino' and pedido.estado != 'listo':
                messages.error(request, 'Solo puedes iniciar entrega cuando el pedido esté listo.')
                return redirect('repartidor_pedidos')
            
            if nuevo_estado == 'entregado' and pedido.estado != 'en_camino':
                messages.error(request, 'Solo puedes marcar como entregado cuando estés en camino.')
                return redirect('repartidor_pedidos')
...
# Logica del reclamo
@login_required
def crear_reclamo_view(request, pk):
    # Buscamos el pedido para asegurarnos que le pertenece al usuario
    pedido = get_object_or_404(Pedido, pk=pk, cliente=request.user)

    # Evitar reclamos duplicados
    reclamo_existente = Reclamo.objects.filter(pedido=pedido, cliente=request.user).first()
    if reclamo_existente:
        messages.error(request, 'Ya has enviado un reclamo para este pedido.')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            reclamo.save()
            messages.success(request, 'Tu reclamo ha sido enviado. Te contactaremos pronto.')
            return redirect('mis_pedidos')
    else:
        form = ReclamoForm()
...
def mis_reclamos_view(request):
    # Buscamos todos los reclamos del usuario actual
...
@login_required
def pos_cajero_view(request):
    """Vista POS exclusiva para cajeros (sin panel admin)"""
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            metodo_pago_obj, created = MetodoPago.objects.get_or_create(
                nombre=metodo_pago_nombre,
                defaults={'tipo': 'efectivo' if metodo_pago_nombre == 'Efectivo' else 'tarjeta', 'activo': True}
            )
...
def cocina_view(request):
    ...
    HU26: Ver todos los pedidos pendientes de preparación
    HU27: Cambiar el estado de un pedido
    """
    # Validar que el usuario sea de cocina o administrador
    if request.user.rol not in ['cocina', 'administrador']:
        messages.error(request, 'No tienes permisos para acceder a esta área.')
        return redirect('home')
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # Validar que el nuevo estado sea válido para cocina
    ...
    # Obtener pedidos pendientes y en preparación (GET)
    pedidos_pendientes = Pedido.objects.filter(
        estado__in=['confirmado', 'en_preparacion']
    ).select_related('cliente', 'metodo_pago').prefetch_related('detalles__producto').order_by('fecha_creacion')

    # Pedidos listos para entregar (últimos 30 minutos)
    hace_30_min = timezone.now() - timedelta(minutes=30)
    pedidos_listos_recientes = Pedido.objects.filter(
        estado='listo',
        fecha_listo__gte=hace_30_min
    ).select_related('cliente', 'metodo_pago').prefetch_related('detalles__producto').order_by('-fecha_listo')

    # Estadísticas para la cocina
    total_pendientes = pedidos_pendientes.filter(estado='confirmado').count()
    total_en_preparacion = pedidos_pendientes.filter(estado='en_preparacion').count()
    ...
    contexto = {
        ...
        'pedidos_listos_recientes': pedidos_listos_recientes,
